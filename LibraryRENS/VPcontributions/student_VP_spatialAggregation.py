...
def distanceToInPossession(rawDict,attributeDict,attributeLabel,TeamAstring,TeamBstring,passDF):
	...
	newAttributes['distanceToInPossession'] = np.nan
...
def angleOpponentToPassline(rawDict,attributeDict,attributeLabel,TeamAstring,TeamBstring,passDF):
	...
	newAttributes['angleOpponentToPassline'] = np.nan
...
def distanceToOpponent(rawDict,attributeDict,attributeLabel,TeamAstring,TeamBstring):
	...
	newAttributes['distanceToOpponent']	= np.nan
...
def distanceToGoal(rawDict,attributeDict,attributeLabel,TeamAstring,TeamBstring,passDF):
	...
	newAttributes['distanceToOpponentGoal']	= np.nan
	newAttributes['distanceToOwnGoal'] = np.nan
...
def ratePlayersPerFeature(rawDict,attributeDict,attributeLabel,TeamAstring,TeamBstring,passDF):
	...
	newAttributes['distanceToOpponentRating'] = np.nan
	newAttributes['angleToPasslineRating'] = np.nan
	newAttributes['distanceToPossessionRating'] = np.nan
	newAttributes['distanceToOpponentGoalRating'] = np.nan
	newAttributes['curPositioningRating'] = np.nan
	...
	for idx,i in enumerate(passDF['PassTimes']):
		...
		if all(curTeamInPossession == TeamAstring):
			...
			minDistanceToGoal = min(curTeamA['distanceToOpponentGoal'])
			maxDistanceToGoal = max(curTeamA['distanceToOpponentGoal'])
			distanceToGoalRating = (curTeamA['distanceToOpponentGoal'] - maxDistanceToGoal) / (minDistanceToGoal - maxDistanceToGoal) * 9 + 1

			curPositioningRating = distanceToPossessionRating * 3.59/10.69 + distanceToGoalRating * 3.26/10.69 + distanceToOpponentRating * 2.34/10.69 + angleRating * 1.5/10.69
			...
			#Rate decrease in distance to goal
			minDistanceToGoal = min(curTeamB['distanceToOpponentGoal'])
			maxDistanceToGoal = max(curTeamB['distanceToOpponentGoal'])
			distanceToGoalRating = (curTeamB['distanceToOpponentGoal'] - maxDistanceToGoal) / (minDistanceToGoal - maxDistanceToGoal) * 9 + 1
