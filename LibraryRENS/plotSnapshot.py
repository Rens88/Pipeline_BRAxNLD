...
if __name__ == '__main__':
	...
	process(outputFilename,currentEvent,teamStrings,TeamAstring,TeamBstring)

def process(outputFilename,currentEvent,teamStrings,TeamAstring,TeamBstring):
	...
	tmp = currentEvent[currentEvent['Ts'] == max(currentEvent['Ts'])]
	if TeamAstring in teamStrings[0]:
		XindA_event = tmp['X'][tmp['TeamID'] == TeamAstring]
		YindA_event = tmp['Y'][tmp['TeamID'] == TeamAstring]
		XindB_event = tmp['X'][tmp['TeamID'] == TeamBstring]
		YindB_event = tmp['Y'][tmp['TeamID'] == TeamBstring]
	elif TeamAstring in teamStrings[1]:
		XindB_event = tmp['X'][tmp['TeamID'] == TeamAstring]
		YindB_event = tmp['Y'][tmp['TeamID'] == TeamAstring]
		XindA_event = tmp['X'][tmp['TeamID'] == TeamBstring]
		YindA_event = tmp['Y'][tmp['TeamID'] == TeamBstring]
	else:
		warn('\nWARNING: Couldnt identify refTeam. Continued with <%s> as refTeam for trialVisualization.\n' %TeamAstring)
		XindA_event = tmp['X'][tmp['TeamID'] == TeamAstring]
		YindA_event = tmp['Y'][tmp['TeamID'] == TeamAstring]
		XindB_event = tmp['X'][tmp['TeamID'] == TeamBstring]
		YindB_event = tmp['Y'][tmp['TeamID'] == TeamBstring]
	...
	# The whole window:
	if TeamAstring in teamStrings[0]:
		XindA = currentEvent['X'][currentEvent['TeamID'] == TeamAstring]
		YindA = currentEvent['Y'][currentEvent['TeamID'] == TeamAstring]
		XindB = currentEvent['X'][currentEvent['TeamID'] == TeamBstring]
		YindB = currentEvent['Y'][currentEvent['TeamID'] == TeamBstring]
	elif TeamAstring in teamStrings[1]:
		XindB = currentEvent['X'][currentEvent['TeamID'] == TeamAstring]
		YindB = currentEvent['Y'][currentEvent['TeamID'] == TeamAstring]
		XindA = currentEvent['X'][currentEvent['TeamID'] == TeamBstring]
		YindA = currentEvent['Y'][currentEvent['TeamID'] == TeamBstring]
	else:
		warn('\nWARNING: Couldnt identify refTeam. Continued with <%s> as refTeam for trialVisualization.\n' %TeamAstring)
		XindA = currentEvent['X'][currentEvent['TeamID'] == TeamAstring]
		YindA = currentEvent['Y'][currentEvent['TeamID'] == TeamAstring]
	...
	# CHANGE to only plot the last point as a dot
	PlotA = plt.plot(XindA_event,YindA_event,".", color = 'r')
	PlotB = plt.plot(XindB_event,YindB_event,".", color = 'b')

	# CHANGE to plot the points leading up to this as a dotted line
