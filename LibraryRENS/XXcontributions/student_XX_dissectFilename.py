...
def process(fname,dataType,TeamAstring,TeamBstring):
	...
	exportData, exportDataString, exportDataFullExplanation,cleanFname,TeamAstring,TeamBstring,Half = KNVB(fname)
	# In dissectFilename.py you can find more advanced examples that use regular expressions.

	return exportData, exportDataString, exportDataFullExplanation,cleanFname,TeamAstring,TeamBstring,Half

def KNVB(fname):
	# Using regular expression to extract info from filename
	regex = r'(\d{8})_([a-zA-Z\s\d]*)_([a-zA-Z\s\d]*)_(\d{1})'
