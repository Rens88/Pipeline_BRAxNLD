...
def processEventAggOnly(df,newStart,newEnd,aggregateEvent,attrLabel_asPanda,aggregatePerPlayer,debuggingMode,dataFolder):
	tTempAgg = time.time()

	eventDescrString = 'During <%ss> seconds up until <%ss> before event <%s>.' %(newStart*-1,newEnd,aggregateEvent)

	# identify the columns that need to be aggregated temporarily
	attrDictCols = [i for i in df.columns if not i in ['Ts','TeamID','PlayerID','X','Y','DataSetIndex',	'TrialBasedIndex',	'eventTimeIndex','eventTime','EventUID','Competition','MatchID'	,'Season','temporalAggregate','dtPrevEvent']]
	attrDictCols_numeric = [tmp for tmp in attrDictCols if tmp not in ['Run', 'Possession/Turnover', 'Pass', 'Goal','OthTeam',	'RefTeam','eventClassification'	, 'eventOverlap'	]]

	# Some temporalAggregation user input
	...
	uniqueEvents = df['EventUID'].unique()

	# Not doing this as we're working with already finished output...
	# SEE ALSO the module <prepareExportStrings>
	...
	for idx,currentEventUID in enumerate(uniqueEvents):
		...
		# and now adjust the window
		tmp1 = dfCurEventOriginal['eventTime'] >= newStart
		tmp2 = dfCurEventOriginal['eventTime'] <= newEnd
		newWindowBoolean = tmp1 & tmp2
		interpolatedCurEventExcerptPanda = dfCurEventOriginal[newWindowBoolean]
	

		eventInstant = interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['eventTime'] == 0,'Ts']
		end_ts = eventInstant.iloc[0] + newEnd
		start_ts = eventInstant.iloc[0] - newStart
		window = (end_ts,start_ts)

		# Determine (and verify) refTeam and othTeam
		refTeam = interpolatedCurEventExcerptPanda['RefTeam'].unique()
		if len(refTeam) != 1:
			warn('\nFATAL WARNING: Multiple refTeams <%s> found for current event in eventAggregate.\nThis is by definintion not possible. Something must have gone wrong in either exporting the eventData, or in selecting the new window.\n' %refTeam)
			exit()
		refTeam = refTeam[0]

		othTeam = interpolatedCurEventExcerptPanda['OthTeam'].unique()
		if len(othTeam) != 1:
			warn('\nFATAL WARNING: Multiple othTeams <%s> found for current event in eventAggregate.\nThis is by definintion not possible. Something must have gone wrong in either exporting the eventData, or in selecting the new window.\n' %othTeam)
			exit()
		othTeam = othTeam[0]

		# Deduce File identifiers
		aggregateString = dfCurEventOriginal['temporalAggregate'][dfCurEventOriginal.index[0]] # should all be the same anyway
		EventUID = dfCurEventOriginal['EventUID'][dfCurEventOriginal.index[0]]
		FileID = EventUID.replace('_' + aggregateString,'')


		# Determine cleanFname
		# if it changes, import targetEvents again.

		if FileID != prev_FileID:
			prev_FileID = FileID # the next iteration can skip loading targetEvents until FileID is different
			...
			# small change because I dont have cleanFname here		
			curFiles = [f[:-4] for f in files if '_preComputed_Event_' in f and FileID in f]
			...
			for key in curKeys:
				...
				for cf in curFiles:
					if key in cf:
						tmp = cf
						break
				targetEventsFname = preComputedTargetFolder + tmp + '.csv'
		...
		overallString = []
		# overallExplanation = []
		for s in FileID.split('_'):

			for k in dfCurEventOriginal.keys():

				if str(dfCurEventOriginal[k][dfCurEventOriginal.index[0]]) == s:
					# found a fileID string
					exportCurrentData.append(s)
					overallString.append(k)

		exportCurrentData.append(dfCurEventOriginal['eventClassification'][dfCurEventOriginal.index[0]])
		overallString.append('eventClassification')

		exportCurrentData.append(dfCurEventOriginal['eventOverlap'][dfCurEventOriginal.index[0]])
		overallString.append('eventOverlap')

		exportCurrentData.append(dfCurEventOriginal['dtPrevEvent'][dfCurEventOriginal.index[0]])
		overallString.append('dtPrevEvent')		

		exportCurrentData.append(dfCurEventOriginal['temporalAggregate'][dfCurEventOriginal.index[0]])
		overallString.append('temporalAggregate')

		exportCurrentData.append(dfCurEventOriginal['EventUID'][dfCurEventOriginal.index[0]])
		overallString.append('EventUID')
		
		exportCurrentData.append(dfCurEventOriginal['RefTeam'][dfCurEventOriginal.index[0]])
		overallString.append('RefTeam')

		exportCurrentData.append(dfCurEventOriginal['OthTeam'][dfCurEventOriginal.index[0]])
		overallString.append('OthTeam')
		...
		# print(currentAsPanda.keys())
		# # print(currentAsPanda)
		# # pdb.set_trace()
		# print()
		# print(exportAsPanda)

		# print(exportAsPanda.shape)
		# print(currentAsPanda.shape)

		# exportAsPanda = pd.concat([exportAsPanda,currentAsPanda],axis = 0,join = 'outer')
	...
	exportMatrix = exportAsPanda[overallString]
	

	# Store new event excerpt panda for plotting? Not strictly necessary, can adjust plotting scripts to take appropriate window from eventAggregate
	# Store output as CSV immediately?

	if debuggingMode:
		elapsed = str(round(time.time() - tTempAgg, 2))
		print('*****----- Time elapsed during temporalAggregation_forEventAggOnly for window <%s> and lag <%s>: %ss' %(newStart*-1,newEnd,elapsed))
...
def process(targetEvents,aggregateLevel,rawDict,attributeDict,exportData,exportDataString,exportFullExplanation,TeamAstring,TeamBstring,debuggingMode,skipEventAgg_curFile,fileIdentifiers,attrLabel,aggregatePerPlayer,includeEventInterpolation,datasetFramerate,eventClassified):
	...
	for idx,currentEvent in enumerate(targetEvents[aggregateLevel[0]]):
		...
		dtPrevEvent = currentEvent[-2] # by definition the second to last.
		exportCurrentData.append(dtPrevEvent) # NB: String and explanation are defined before the for loop
		...
		## INCLUDE event columns here
		# and then, put countEvents2 in aggregateCurrentEvent
		# Test for template. (with skipEventAgg = False)
		# Close your eyes and go.

		# attrDictCols2 = [i for i in attributeDict.columns if not i in ['Ts','TeamID','PlayerID','X','Y']] # already in rawDict; 
		...
		if includeEventInterpolation:
			warn('\nWARNING: This doesnt work anymore now that all attributeDict columns have been concateneated in curEventExcerptPanda.\nInclude <attrDictCols> in interpolateEventExcerpt to only look at numeric columns.\n and/or write interpolation code for non-numeric columns (eventcolumns).\n')
...
def prepareExportStrings(exportDataString,aggregateLevel,exportFullExplanation):
	...
	exportDataString.append('dtPrevEvent')
	exportFullExplanation.append('Time (in seconds) between the instant (tEnd) of the previous and the current event.')
...
def userInput():
	...
	aggrMeth_popLevel = ['avg', 'min','max', 'std'] #LT: count belangrijk om te zien of die over time of player heen gaat. Bij time gelijk aan aantal timestamps
...
def aggregateTemporallyINCEPTION(population,aggregationOrder,aggrMeth_popLevel,aggrMeth_playerLevel,curEventExcerptPanda,key,refTeam,othTeam,attrLabel,eventDescrString, exportCurrentData, overallString, overallExplanation):
	...
	#LT: added! NOT GENERIC
	keysThatShouldNotBeAggregatedForOtherTeam = ['zone', 'inZone', 'opponentsHalf', 'inPenaltyArea','Link_Control', 'velRelToBall', 'velRelToBallSquared','Link_Pressure','minDistToDef','avgDistToDef2','avgDistToDef3','minAngleInPossDefGoal','avgAngleInPossDefGoal2','avgAngleInPossDefGoal3','Link_Density', 'Link_SDPlayerWithBall', 'Link_PDPlayerWithBall', 'angleToGoal', 'majority', 'centrality', 'distToGoal','dangerousity','DA_ZOCOPR','DA_ZOCODE','DA_ZOPRDE','DA_COPRDE','DA_ZOCO','DA_ZOPR','DA_ZODE','DA_COPR','DA_CODE','DA_PRDE']
	if population == 'refTeam' or key not in keysThatShouldNotBeAggregatedForOtherTeam:
...
def aggregateTemporally(data,*positional_parameters,**keyword_parameters):
	...
	if not exportNans:
		...
		dataTemp = data[data > 0]
		if dataTemp.size == 0:
			avgGreaterThanZero = 0
			minValGreaterThanZero = 0
		else:
			avgGreaterThanZero = np.nanmean(dataTemp, axis = axis)
			minValGreaterThanZero = np.min(dataTemp, axis = axis)
		...
		kur = np.nan
		ske = np.nan
		avgGreaterThanZero = np.nan
		minValGreaterThanZero = np.nan		
	...
	if 'avg' in aggrMethods:
		...
		exportCurrentData.append(avgGreaterThanZero)
		overallString.append(curKeyString + '_avgGreaterThanZero')				
		overallExplanation.append('Average of <%s> (%s) with only values greater than zero. For <%s>. %s' %(curKeyString, curLabel, targetGroup, eventDescrString))
	...
	if 'min' in aggrMethods:
		...
		exportCurrentData.append(minValGreaterThanZero)
		overallString.append(curKeyString + '_minGreaterThanZero')
		overallExplanation.append('Minimum of <%s> (%s) with only values greater than zero. For <%s>. %s' %(curKeyString, curLabel, targetGroup, eventDescrString))
...
def aggregateCurrentEvent(attrDictCols_numeric,window,interpolatedCurEventExcerptPanda,refTeam,othTeam,ref,oth,exportCurrentData,aggrMeth_popLevel,aggrMeth_playerLevel,aggregatePerPlayer,aggregationOrders,overallString,optionalInput,populations,eventDescrString,targetEvents,aggregateEvent):

	itWiOvEvAg_onlyUseForWarning = False
	if 'attributeDict' in optionalInput.keys():
		attributeDict = optionalInput['attributeDict']
	else:
		attributeDict = interpolatedCurEventExcerptPanda
		# This only happens if iterateWindowsOverEventAgg ....
		itWiOvEvAg_onlyUseForWarning = True

	if 'rawDict' in optionalInput.keys():
		rawDict = optionalInput['rawDict']
	else:
		rawDict = interpolatedCurEventExcerptPanda
	...
	if 'overallExplanation' in optionalInput.keys():
		overallExplanation = optionalInput['overallExplanation']
	else:
		overallExplanation = []		

	if 'attrLabel' in optionalInput.keys():
		attrLabel = optionalInput['attrLabel']
	else:
		attrLabel = []		
	if 'rowswithinrange' in  optionalInput.keys():
		rowswithinrange = optionalInput['rowswithinrange']
	else:
		rowswithinrange = interpolatedCurEventExcerptPanda.index
	...
	exportCurrentData,overallString,overallExplanation =\
	countEvents2.goals(window,aggregateEvent,targetEvents,refTeam,othTeam,exportCurrentData,overallString,overallExplanation)
	
	exportCurrentData,overallString,overallExplanation =\
	countEvents2.turnovers(window,aggregateEvent,targetEvents,refTeam,othTeam,exportCurrentData,overallString,overallExplanation)

	exportCurrentData,overallString,overallExplanation =\
	countEvents2.possessions(window,aggregateEvent,targetEvents,refTeam,othTeam,exportCurrentData,overallString,overallExplanation)
	
	exportCurrentData,overallString,overallExplanation =\
	countEvents2.passes(window,aggregateEvent,targetEvents,refTeam,othTeam,exportCurrentData,overallString,overallExplanation)
	...
	for key in attrDictCols_numeric:
		...
		if all(attributeDict[key].isnull()):
			# No data at all..
			if not itWiOvEvAg_onlyUseForWarning:
		...
		curContent = pd.isnull(interpolatedCurEventExcerptPanda[key]) == False #LT: added! curContent = np.isnan(interpolatedCurEventExcerptPanda[key]) == False			
		...
		if not any(curContent):
			...
			nPlREF = len(interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == refTeam,'PlayerID'].unique())
			nPlOTH = len(interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == othTeam,'PlayerID'].unique())

			if nPlREF < 3 or nPlOTH < 3:
				...
				for iREF in interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == refTeam,'PlayerID'].unique():
					# if there is a player that only had 1 frame and the nPlREF is only 4, then it is treated similarly as nPlREF < 3
					if sum(interpolatedCurEventExcerptPanda['PlayerID'] == iREF) == 1 and nPlREF == 4:
						safeEnoughToBeAgroupRow = True
				...
				if not safeEnoughToBeAgroupRow: # only necessary if it hasn't yet been found
					for iOTH in interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == othTeam,'PlayerID'].unique():
						# if there is a player that only had 1 frame and the nPlREF is only 4, then it is treated similarly as nPlREF < 3
						if sum(interpolatedCurEventExcerptPanda['PlayerID'] == iOTH) == 1 and nPlOTH == 4:
							safeEnoughToBeAgroupRow = True
							warn('\nWARNING: Assumed that if there is only 1 occurence for a player, it was impossible to compute some of the team variables. (such as surface etc.)\n')
							break
				...
				if not safeEnoughToBeAgroupRow: # only necessary if it hasn't yet been found
					...
					tmp = interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == refTeam,['Ts','PlayerID']]
				...
				# check how many unique players exist at each timepoint, same for Team B
					tmp = interpolatedCurEventExcerptPanda.loc[interpolatedCurEventExcerptPanda['TeamID'] == othTeam,['Ts','PlayerID']]
					tmp = tmp.pivot(columns ='Ts', values='PlayerID')
					for c in tmp.keys():
						tmp1 = tmp.loc[tmp[c].notnull(),c]
		...
		if all(tmpPlayerID == 'ball'): 
			if targetGroup == []:				
				...
				print('number of players of team REF within the excerpt nPlREF = %s' %nPlREF)
				print('number of players of team OTH within the excerpt nPlOTH = %s' %nPlOTH)
				pdb.set_trace()
				exit()
