...
#########################################################################
def process(dirtyFname,cleanFname,dataType,dataFolder,cleanedFolder,spatAggFname,spatAggFolder,eventAggFolder,eventAggFname,TeamAstring,TeamBstring,headers,readAttributeCols,timestampString,readEventColumns,conversionToMeter,skipCleanup,skipSpatAgg,skipEventAgg,exportData, exportDataString,includeCleanupInterpolation,datasetFramerate,debuggingMode,skipComputeEvents,aggregateLevel,csvFolder):
	...
	# if cleanFname in cleanFnames and skipCleanup:
		...
		if 'fatalIssue' in fileHeaders:
			...
			logging.warning(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' - ' + basename(__file__) + ' - FATAL WARNING: In a previous clean-up, there was a problem with timestamp: Some timestamps occurred more often than there were unique <PlayerID>s')
		else:
		...
		if dataType == "NP":
			...
			student_LT_cleanUp.process(dirtyFname,cleanFname,dataFolder,cleanedFolder,headers,readAttributeCols,debugOmittedRows,readEventColumns,TeamAstring,TeamBstring,csvFolder)
		else:
		...
		if fatalTeamIDissue:
			...
			df_Fatal.to_csv(cleanedFolder + cleanFname)
			fatalIssue = True
			logging.warning(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' - ' + basename(__file__) + '\nSomething with the same timestamp occurring multiple times for the same playerID\nFatal cleanup issue result in the current file being skipped in its entirety\n')
			...
			df_Fatal.to_csv(cleanedFolder + cleanFname)
			fatalIssue = True
			logging.warning(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' - ' + basename(__file__) + '\nProblems with groupRows. Perhaps one of the players did not have a team assigend to it.\nFatal cleanup issue result in the current file being skipped in its entirety\n')
