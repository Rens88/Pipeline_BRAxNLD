...
def setPlayerID(df,ID,Tid,ts):
	...
	if diff > 0:
		messagebox.showerror('Kan spelers niet onderscheiden', 'Niet alle PlayerIDs zijn uniek. \n\nOPLOSSING: Zorg dat alle spelers een unieke ID hebben, of een uniek shirtnummer per team.')
		logging.critical(datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' - ' + basename(__file__) + '\nNot all playerIDs are unique! This will cause problems in spatial and temporal aggregation. There are %s duplicated values.' %(diff))
